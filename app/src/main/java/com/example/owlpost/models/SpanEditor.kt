...
class SpanEditor(private val builder: SpannableStringBuilder) {
    /**
     ...
     *  If size to remove less than selection range then makes new spans
     */
    fun removeAllSpans(selectionStart: Int, selectionEnd: Int){
        val spans = builder.getSpans(selectionStart, selectionEnd, ParcelableSpan::class.java)
        for (span in spans){
            removeSpan(selectionStart, selectionEnd, span)
        }
    }
